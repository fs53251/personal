...
namespace RPPP_WebApp.Controllers {
    public class IncidentController : Controller {
        ...
        public IncidentController(RPPP04Context ctx, IOptionsSnapshot<AppSettings> options, ILogger<AutocestaController> logger) {
            this.ctx = ctx;
        ...
        public async Task<IActionResult> Create(Incident incident) {
            logger.LogTrace(JsonSerializer.Serialize(incident));
            if (ModelState.IsValid) {
                try {
                    ctx.Add(incident);
                    await ctx.SaveChangesAsync();

                    logger.LogInformation(new EventId(1000), $"Incident {incident.Id} dodan.");
                    TempData[Constants.Message] = $"Incident {incident.Id} dodana.";
                    TempData[Constants.ErrorOccurred] = false;
                    return RedirectToAction(nameof(Index));
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom dodavanje novog incidenta: {0}", exc.CompleteExceptionMessage());
        ...
        public async Task<IActionResult> Delete(int id) {
            ...
            if (incident != null) {
                try {
                    ctx.Remove(incident);
                    await ctx.SaveChangesAsync();
                    logger.LogInformation($"Incident: sa šifrom {id} uspješno obrisan");
                    responseMessage = new ActionResponseMessage(MessageType.Success, $"Incident sa šifrom {id} uspješno obrisan.");
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom brisanja incidenta: " + exc.CompleteExceptionMessage());
        ...
        public async Task<IActionResult> Edit(Incident incident) {
            ...
            if (ModelState.IsValid) {
                try {
                    ctx.Update(incident);
                    logger.LogInformation($"Incident: uspješno ažuriran");
                    await ctx.SaveChangesAsync();
                    return RedirectToAction(nameof(Get), new { id = incident.Id });
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom ažuriranja incidenta: " + exc.CompleteExceptionMessage());
                    ModelState.AddModelError(string.Empty, exc.CompleteExceptionMessage());
                    await PrepareDropDownList();
                    return PartialView(incident);
                }
