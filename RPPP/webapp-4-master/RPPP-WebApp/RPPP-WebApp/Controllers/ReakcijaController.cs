...
namespace RPPP_WebApp.Controllers {
    public class ReakcijaController : Controller {
        ...
        public ReakcijaController(RPPP04Context ctx, IOptionsSnapshot<AppSettings> options, ILogger<DionicaController> logger) {
        ...
        public async Task<IActionResult> Create(Reakcija reakcija) {
            if (ModelState.IsValid) {
                try {
                    ...
                    logger.LogInformation($"Reakcija: {reakcija.Id} uspješno ažurirana");
        ...
        public async Task<IActionResult> Delete(int id) {
            ...
            if (reakcija != null) {
                try {
                    ctx.Remove(reakcija);
                    await ctx.SaveChangesAsync();
                    logger.LogInformation($"Reakcija: {id} uspješno obrisana");
                    responseMessage = new ActionResponseMessage(MessageType.Success, $"Reakcija sa šifrom {id} uspješno obrisana.");
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom brisanja reakcije: " + exc.CompleteExceptionMessage());
                ...
                logger.LogError("Pogreška prilikom brisanja reakcije, ne postoji: " );
        ...
        public async Task<IActionResult> Edit(Reakcija reakcija) {
            ...
            if (ModelState.IsValid) {
                try {
                    ctx.Update(reakcija);
                    logger.LogInformation($"Reakcija uspješno ažurirana");
                    await ctx.SaveChangesAsync();
                    return RedirectToAction(nameof(Get), new { id = reakcija.Id });
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom ažuriranja reakcije: " + exc.CompleteExceptionMessage());
                    ModelState.AddModelError(string.Empty, exc.CompleteExceptionMessage());
                    await PrepareDropDownLists();
