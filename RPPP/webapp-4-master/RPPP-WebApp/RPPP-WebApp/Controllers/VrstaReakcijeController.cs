...
namespace RPPP_WebApp.Controllers {
    public class VrstaReakcijeController : Controller {
        ...
        public VrstaReakcijeController(RPPP04Context ctx, IOptionsSnapshot<AppSettings> options, ILogger<AutocestaController> logger) {
            this.ctx = ctx;
        ...
        public async Task<IActionResult> Create(VrstaReakcije vrstaReakcije) {
            logger.LogTrace(JsonSerializer.Serialize(vrstaReakcije));
            if (ModelState.IsValid) {
                try {
                    ctx.Add(vrstaReakcije);
                    await ctx.SaveChangesAsync();
                    logger.LogInformation(new EventId(1000), $"Vrsta reakcije {vrstaReakcije.Naziv} dodana.");
                    TempData[Constants.Message] = $"Vrsta reakcije {vrstaReakcije.Naziv} dodana.";
                    TempData[Constants.ErrorOccurred] = false;
                    return RedirectToAction(nameof(Index));
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom dodavanje nove vrste reakcije: {0}", exc.CompleteExceptionMessage());
        ...
        public async Task<IActionResult> Delete(int id) {
            ...
            if (vrstaReakcije != null) {
                try {
                    ...
                    logger.LogInformation($"Vrsta reakcije: {naziv} uspješno obrisana");
                    ...
                    logger.LogError("Pogreška prilikom brisanja vrste reakcije: " + exc.CompleteExceptionMessage());
        ...
        public async Task<IActionResult> Edit(VrstaReakcije vrstaReakcije) {
            ...
            if (ModelState.IsValid) {
                try {
                    ...
                    logger.LogInformation($"Vrste reakcije: {vrstaReakcije.Id} uspješno ažurirana");
                    await ctx.SaveChangesAsync();
                    return RedirectToAction(nameof(Get), new { id = vrstaReakcije.Id });
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom ažuriranja vrste reakcije: " + exc.CompleteExceptionMessage());
                    ModelState.AddModelError(string.Empty, exc.CompleteExceptionMessage());
                    return PartialView(vrstaReakcije);
                }
            } else {
