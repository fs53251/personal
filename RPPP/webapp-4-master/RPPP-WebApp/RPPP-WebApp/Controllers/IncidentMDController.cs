...
namespace RPPP_WebApp.Controllers {
    public class IncidentMDController : Controller {
        ...
        public IncidentMDController(RPPP04Context ctx, IOptionsSnapshot<AppSettings> options,
            ILogger<AutocestaMDController> logger) {
            this.ctx = ctx;
        ...
        public async Task<IActionResult> Create(Incident incident) {
            if (ModelState.IsValid) {
                try {
                    ...
                    logger.LogInformation(new EventId(1000), $"MD incident  {incident.Id} dodan.");
                    TempData[Constants.Message] = $"Incident {incident.Id} dodana.";
                    TempData[Constants.ErrorOccurred] = false;
                    return RedirectToAction(nameof(Index));
                } catch (Exception exc) {
                    logger.LogError("Pogreška prilikom dodavanja novog incidenta: {0}", exc.CompleteExceptionMessage());
        ...
        public async Task<IActionResult> Delete(int id) {
            ...
            if (incident != null) {
                try {
                    ctx.Remove(incident);
                    await ctx.SaveChangesAsync();
                    logger.LogInformation($"Incidenta sa šifrom {id} uspješno obrisana");
        ...
        public async Task<IActionResult> Edit(Incident incident) {
            ...
            if (!checkId) {
                logger.LogWarning("Ne postoji traženi id incidenta");
                return NotFound($"Neispravan id incidenta: {incident?.Id}");
            }

            if (ModelState.IsValid) {
                try {
                    ctx.Update(incident);
                    logger.LogInformation($"(MD) incidenta uspješno ažuriran");
                    await ctx.SaveChangesAsync();
